...
void init(particle_t p, u64 n)
{
  ...
  #pragma omp  parallel for num_threads(nb) private(k)
  for (u64 i = 0; i < n; i++)
    {
      //
      u64 r1 = (u64)rand();
...
void move_particles(particle_t p, const f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i++)
    ...
    const f32 d_2_o=d_2*d_2*d_2;
    const f32 d_3_over_2=sqrt(d_2_o);
...
x = *(f64*)&i; // convert bits back to double
...
int main(int argc, char **argv)
{
  ...
  f64 x=(steps - warmup) - (rate * rate);
  
  drate = sqrt(drate)*InvSqrt(x);
